...
这篇文章和前面两篇（Hexo 建站、因时电缸控制）看起来毫不相关，但对我来说，它们指向同一个方向：**在机器人领域做研究，不能只待在实验室里调 PID。** 追踪 AI Agent 基础设施的进展、上手内测产品、记录实测结果，和掌握一种电机控制协议、搭建一个个人网站一样，都是构建完整技术视野的必经之路。

Agently Mail 目前还在内测，CLI 多账号管理不完善、平台接入有限、部分操作还需手动切换——但这些恰恰说明我来得足够早。记录下这个阶段的状态，等它成长起来回头看，价值比等成熟了再写一篇"使用教程"大得多。
