...
这是目前最大的体验摩擦点。`agently-cli` 的 OAuth Token 保存在全局配置中，即使 Claude Code 和 Hermes 是独立的 Agent 进程，底层调用的仍是同一个全局 CLI。这导致切换邮箱时必须 `auth logout → auth login` 手动重新授权。对多 Agent 场景来说，理想状态是每个 Agent 进程维护独立的 Token 上下文，相信这是后续版本的重点优化方向。
