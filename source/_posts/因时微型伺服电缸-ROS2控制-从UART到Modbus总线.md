...
<table>
<tr><th width="25%">参数</th><th width="25%">数值</th><th width="25%">参数</th><th width="25%">数值</th></tr>
<tr><td>行程</td><td>50 mm</td><td>最大推拉力</td><td>50 N</td></tr>
<tr><td>重量</td><td>50 g</td><td>堵转推拉力</td><td>80 N</td></tr>
<tr><td>空载速度</td><td>17 mm/s</td><td>额定速度</td><td>8 mm/s</td></tr>
<tr><td>重复定位精度</td><td>±0.1 mm</td><td>最大自锁力</td><td>80 N</td></tr>
<tr><td>力传感器范围</td><td>±100 N</td><td>力传感器分辨率</td><td>1 N</td></tr>
<tr><td>静态电流</td><td>0.05 A</td><td>峰值电流</td><td>2 A</td></tr>
<tr><td>供电电压</td><td>DC 8V</td><td>通信接口</td><td>D 型（UART, LVTTL 3.3V）</td></tr>
<tr><td>防护等级</td><td>IP40</td><td>控制模式</td><td>位置/速度/力/电压/伺服/速度力控</td></tr>
</table>
...
UART 直连模式下，每台电机独占一个 USB 串口，NUC 主机 USB 接口数量构成硬约束。当平台自由度超过该数量时，需要扩展 USB 或切换通信架构。
...
| 层次 | UART 版 | Modbus 版 |
|------|---------|-----------|
| 通信层 | `ins_uart.py`（`pyserial`，字节流收发） | `ins_modbus.py`（`minimalmodbus`，寄存器读写） |
| 协议层 | `ins_protocol.py`（因时自定义二进制帧） | `ins_modbus_protocol.py`（Modbus 寄存器地址映射） |
| 驱动层 | `motor_driver.py`（模式封装、状态结构体） | `motor_driver.py`（API 不变） |
